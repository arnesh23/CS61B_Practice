...
public class SList {
    ...
    public void squish() {
        // Solution written by Arnesh Regmi
        // Fill in your solution here.  (Ours is eleven lines long.)

        if(this.head == null)           //Empty List
            ;

        if(this.size >= 2) {            //takes at least two nodes to squish()
            SListNode curr = this.head;
            SListNode next = curr.next;

            while(curr.next != null) {
                if (curr.item.equals(next.item)) {
                    curr.next = next.next;
    ...
    public void twin() {
        ...
        if(this.size >= 1){            // takes at least 1 Node to twin
            SListNode curr = this.head;
            SListNode next = curr.next;

            while(curr != null){
                next = curr.next;
                SListNode newNode = new SListNode(curr.item);
                curr.next = newNode;
                newNode.next = next;
                curr = newNode.next;
                this.size = this.size+1;
